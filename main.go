...
func main() {
	...
	var args struct {
		...
		Generate *struct {
			...
			pinArg
			managementArg
			Subject string `arg:"env:GENERATE_SUBJECT" help:"x509 Subject for bogus certificate commonName" default:"ECDSA signing key generated by yubecdsa"` // TODO allow specifying funky /O=.../C=.../ syntax for more than just commonName?
	...
	case args.Generate != nil:
		...
		certTemplate := x509.Certificate{
			...
			// TODO more fields? https://pkg.go.dev/crypto/x509#Certificate and https://pkg.go.dev/crypto/x509#CreateCertificate

			// set Issuer to something that identifies yubecdsa as the tool that triggered the key generation 👀
			// TODO allow this to be overridden in a flag?
			Issuer: pkix.Name{CommonName: "THIS x509 IS INVALID; the associated slot stores a key generated by https://github.com/tianon/yubecdsa and all parts of this x509 are purely bogus informational metadata about that key"},
			// TODO add a flag that makes the x509 more valid/compliant?

			Subject: pkix.Name{CommonName: sub.Subject},

			// TODO allow overriding this via flag?
		...
		// if template == parent, CreateCertificate assumes we're looking for self-signed, which is true, but we want to set issuer too, and issuer comes from parent's subject (so if we want to set Issuer and Subject to separate values, we have to set Subject on the parent separate from the template)
		certParent := certTemplate
		certParent.Subject = certTemplate.Issuer

		certRaw, err := x509.CreateCertificate(nil, &certTemplate, &certParent, pub, priv)
		if err != nil {
			log.Fatalf("failed to CreateCertificate: %v", err)
		}

		err = yubi.SetCertificate(sub.ManagementKey, slot, &x509.Certificate{Raw: certRaw}) // SetCertificate takes an x509.Certificate argument, but only reads Raw from it
		if err != nil {
